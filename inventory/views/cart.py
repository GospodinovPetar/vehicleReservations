...
def view_cart(request):
    cart = Cart.get_or_create_active(request.user)
    items = list(cart.items.select_related("vehicle", "pickup_location", "return_location"))
    rows = []
    for it in items:
        q = quote_total(it.start_date, it.end_date, RateTable(day=float(it.vehicle.price_per_day), currency="EUR"))
...
def my_reservations(request):
    ...
    canceled = (
        Reservation.objects
        .filter(user=request.user, status=ReservationStatus.CANCELED)
...
def cancel_group(request, group_id):
    ...
    with transaction.atomic():
        ...
        updated = group.reservations.filter(cancelable).update(status=ReservationStatus.CANCELED)
        group.delete()
