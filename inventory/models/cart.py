...
class ReservationGroup(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='reservation_groups')
    reference = models.CharField(max_length=32, unique=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    def __str__(self): return f"{self.reference or self.pk}"
...
class CartItem(models.Model):
    cart = models.ForeignKey('inventory.Cart', on_delete=models.CASCADE, related_name='items')
    vehicle = models.ForeignKey('inventory.Vehicle', on_delete=models.CASCADE)
    start_date = models.DateField()
    end_date = models.DateField()
    pickup_location = models.ForeignKey('inventory.Location', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    return_location = models.ForeignKey('inventory.Location', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    ...
    def clean(self):
        ...
        available_ids = set(Reservation.available_vehicles(
        ...
        if self.vehicle.pk not in available_ids:
            raise ValidationError({'vehicle': 'This vehicle is not available for the selected period.'})
