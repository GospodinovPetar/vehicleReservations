...
class Reservation(models.Model):
    ...
    vehicle = models.ForeignKey('inventory.Vehicle', on_delete=models.PROTECT, related_name='reservations')
    pickup_location = models.ForeignKey('inventory.Location', on_delete=models.PROTECT, related_name='+')
    ...
    group = models.ForeignKey('inventory.ReservationGroup', on_delete=models.SET_NULL, null=True, blank=True, related_name='reservations')
    ...
    def clean(self):
        ...
        if self.vehicle.pk and self.start_date and self.end_date:
            overlapping = (
                Reservation.objects
                .filter(vehicle_id=self.vehicle.pk, status__in=BLOCKING_STATUSES)
    ...
    def _compute_total_price(self) -> Decimal:
        ...
        daily = getattr(self.vehicle, 'price_per_day', Decimal('0.00')) or Decimal('0.00')
